	...
	for( unsigned int i = 0 ; i < names.size() ; ++i ) {
		if ( nr_of_genes[i] >= cutoff ) { 
			...
			while ( it != smallest_rand_p_l.end() && 
				*it <= data_pvals_l[i] + 1.0e-13) // NEW: add tolerance to account for float inaccuracy
					n_l++, it++ ;
			int n_g = 0 ;
			it = smallest_rand_p_g.begin() ;
			while ( it != smallest_rand_p_g.end() && 
				*it <= data_pvals_g[i] + 1.0e-13) // NEW: add tolerance to account for float inaccuracy 
					n_g++, it++ ;
