...
rearrange_output=function(summary,cluster,term){
	...
	freqs=as.data.frame(table(pure[pure[,4] < 0.05,c("age_category","cate")]))
	colnames(freqs)[3]="times_FWER_under_0.05"
	# lowest FWER
	lowest=aggregate(pure[,4], by=list(age_category=pure$age_category,cate=pure$cate),min)
	colnames(lowest)[3]="min_FWER"
	# mean FWER
	meany=aggregate(pure[,4], by=list(age_category=pure$age_category,cate=pure$cate),mean)
	colnames(meany)[3]="mean_FWER"	
	
	## collapse FWER over cutoffs
	fwers=aggregate(1:nrow(pure), by=list(cate=pure$cate,age_category=pure$age_category), function(x) paste(pure[x,4][order(pure[x,"cutoff_quantile"])],collapse=";"))
	...
	# order
	out=preout[order(preout$age_category, -1*(preout$times_FWER_under_0.05),preout$mean_FWER,preout$min_FWER),-1]
	# order rownames 
